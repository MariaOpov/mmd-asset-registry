# MMD Asset Registry v0.9.5.3 Release Checklist

The Git/GitHub release label is `v0.9.5.3`; the PEP 440 runtime and
distribution version is `0.9.5.3`. This is a normal GitHub Release, not a prerelease.
...
- [ ] Confirm final package version and unchanged registry schemas:
  ...
  python -c "from mmd_registry import __version__; from mmd_registry.constants import LATEST_SCHEMA_VERSION, SUPPORTED_SCHEMA_VERSIONS; assert __version__ == '0.9.5.3'; assert LATEST_SCHEMA_VERSION == '0.3'; assert SUPPORTED_SCHEMA_VERSIONS == frozenset(('0.2', '0.3'))"
  ```
...
- [ ] Run Ruff, compilation, compatibility/public-boundary checks, and the full
  ...
  rem Record the observed v0.9.5.3 full-suite count, skip count, and coverage from this run.
...
- [ ] Build fresh artifacts, inspect them, and verify a disposable clean wheel
  ...
  rem Record the observed v0.9.5.3 wheel/sdist member counts from this fresh build.
...
- [ ] Confidence is not part of v0.9.5.3; ambiguity presentation remains
  deferred to v0.9.5.4 and the Smart CLI remains deferred to v0.9.5.5.
...
- [ ] Push only after the final v0.9.5.3 release-certification checkpoint and
...
- [ ] Confirm `main == origin/main`, tree clean, and no local/remote tag or
  GitHub Release already uses `v0.9.5.3`:
  ...
  git --no-pager rev-parse HEAD
  git --no-pager rev-parse origin/main
  git --no-pager tag --list v0.9.5.3
  git ls-remote --tags origin refs/tags/v0.9.5.3 refs/tags/v0.9.5.3^{}
  gh release view v0.9.5.3 --json tagName,name,url,isDraft,isPrerelease,publishedAt,targetCommitish
  ```
...
- [ ] On verified merged `main` only, and only after explicit Maintainer
  ...
  ```bat
  git tag -a v0.9.5.3 -m "MMD Asset Registry v0.9.5.3"
  git --no-pager show v0.9.5.3 --no-patch --format=fuller
...
- [ ] Verify the remote annotated tag resolves to the intended merged-main
  ...
  git --no-pager rev-parse "v0.9.5.3^{}"
  git ls-remote --tags origin refs/tags/v0.9.5.3 refs/tags/v0.9.5.3^{}
  ```

## 7. Normal GitHub Release

- [ ] Review `v0.9.5.3` release notes and create a normal GitHub Release from the
  verified remote tag, only after explicit Maintainer authorization:

  ```bat
  gh release create v0.9.5.3 --verify-tag --title "MMD Asset Registry v0.9.5.3" --notes-file "%USERPROFILE%\Downloads\v0.9.5.3-release-notes.md"
  ```
...
- [ ] Confirm `isDraft` is `false`, `isPrerelease` is `false`, tag is `v0.9.5.3`,
...
- [ ] Confirm local `main`, `origin/main`, dereferenced annotated tag `v0.9.5.3`,
  and the normal GitHub Release identify the same intended release commit.
- [ ] Confirm release notes state package version `0.9.5.3`, both passing CI
