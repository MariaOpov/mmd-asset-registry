# MMD Asset Registry v0.9.5.9 Release Checklist

The Git/GitHub release label is `v0.9.5.9`; the PEP 440 runtime and
distribution version is `0.9.5.9`. This is a normal GitHub Release, not a prerelease.
...
- [ ] Confirm final package version and unchanged registry schemas:
  ...
  python -c "from mmd_registry import __version__; from mmd_registry.constants import LATEST_SCHEMA_VERSION, SUPPORTED_SCHEMA_VERSIONS; assert __version__ == '0.9.5.9'; assert LATEST_SCHEMA_VERSION == '0.3'; assert SUPPORTED_SCHEMA_VERSIONS == frozenset(('0.2', '0.3'))"
...
- [ ] Build fresh artifacts, inspect them, and verify a disposable clean wheel
  ...
  rem Record the observed v0.9.5.9 wheel/sdist member counts from this fresh build.
...
- [ ] Smart Material Color Draft is private draft composition only. Exact
  MATERIAL evidence and non-ambiguous semantics are required; confidence is not
  permission. RGB drafting preserves source alpha and adds no apply, source
  write, writer/remapper, structural schema widening, CLI mutation command, or
  publication authority.
- [ ] Smart Material Preview Bridge is private preview delegation only. It
  consumes a certified source-bound draft, preserves source bytes/alpha,
  and adds no second preview engine/DTO, apply, writer/remapper, CLI
  mutation command, package-root export, or publication authority.
- [ ] Confirmed Smart Material Apply Integration is private confirmed execution
  only. Explicit confirmation binds exact preview/source/plan identity; apply
  delegates to the existing no-clobber generic authority, preserves the source,
  reparses/certifies the destination, and adds no second writer/remapper/apply
  engine, overwrite option, Smart mutation CLI, or public API.
- [ ] Smart Material Appearance Expansion is private bounded authoring only.
  Exact MATERIAL evidence is required; transparency/specular/edge draft through
  the existing schema-one `UpdateMaterial` authority. Texture replacement and
  appearance presets remain architecture-blocked, brightness remains unsupported
  for v0.9.5.9, and the expansion adds no automatic apply, Smart mutation CLI,
  new transaction schema, second writer/remapper/preview/apply/publication path,
  or public API.
...
- [ ] Push only after the final v0.9.5.9 release-certification checkpoint and
...
- [ ] Confirm `main == origin/main`, tree clean, and no local/remote tag or
  GitHub Release already uses `v0.9.5.9`:
  ...
  git --no-pager rev-parse HEAD
  git --no-pager rev-parse origin/main
  git --no-pager tag --list v0.9.5.9
  git ls-remote --tags origin refs/tags/v0.9.5.9 refs/tags/v0.9.5.9^{}
  gh release view v0.9.5.9 --json tagName,name,url,isDraft,isPrerelease,publishedAt,targetCommitish
  ```
...
- [ ] On verified merged `main` only, and only after explicit Maintainer
  ...
  ```bat
  git tag -a v0.9.5.9 -m "MMD Asset Registry v0.9.5.9"
  git --no-pager show v0.9.5.9 --no-patch --format=fuller
  git push origin v0.9.5.9
  ```
...
- [ ] Verify the remote annotated tag resolves to the intended merged-main
  ...
  git --no-pager rev-parse "v0.9.5.9^{}"
  git ls-remote --tags origin refs/tags/v0.9.5.9 refs/tags/v0.9.5.9^{}
  ```

## 7. Normal GitHub Release

- [ ] Review `v0.9.5.9` release notes and create a normal GitHub Release from the
  verified remote tag, only after explicit Maintainer authorization:

  ```bat
  gh release create v0.9.5.9 --verify-tag --title "MMD Asset Registry v0.9.5.9" --notes-file "%USERPROFILE%\Downloads\v0.9.5.9-release-notes.md"
  ```
...
- [ ] Confirm `isDraft` is `false`, `isPrerelease` is `false`, tag is `v0.9.5.9`,
...
- [ ] Confirm local `main`, `origin/main`, dereferenced annotated tag `v0.9.5.9`,
  and the normal GitHub Release identify the same intended release commit.
- [ ] Confirm release notes state package version `0.9.5.9`, both passing CI
